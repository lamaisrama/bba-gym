...
public class MypageBusinessServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//시설 등록 현황 가져오기
		List<CenterEnroll> ce = new MyPageService2().findMyCenter(mCode);
		for(int i=0;i<ce.size();i++) {
			if(ce.get(i).getApproval()==null) {
				ce.get(i).setApproval("진행 중");
			}else if(ce.get(i).getApproval().equals("Y")) {
				ce.get(i).setApproval("승인");
			}else if(ce.get(i).getApproval().equals("N")) {
				ce.get(i).setApproval("거절");
			}
		}
		//트레이너 등록 현황 가져오기
