		...
		<div class="col-lg-8 col-md-8 col-sm-12">
			...
			<div>
				<form action="<%=request.getContextPath()%>/member/searchId.do"method="post" onsubmit="return fn_enroll_validate();">
					...
					<br><button type="button"class="submit" onclick="location.replace('<%=request.getContextPath()%>/member/logoutgologin.do')">로그인페이지로 이동</button>
				<br><button type="button"class="submit" onclick="location.replace('<%=request.getContextPath()%>/member/logout.do')">메인 화면으로 돌아가기</button>
				
				<br>
				<button type="button"class="submit" onclick="location.replace('<%=request.getContextPath()%>/member/pw.do')">비밀번호 찾기</button>
				<br>
